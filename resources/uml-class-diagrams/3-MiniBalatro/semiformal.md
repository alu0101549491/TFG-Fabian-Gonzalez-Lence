...
Perfect! I've created the UML class diagram for the Balatro-inspired card game.

## 📊 View Balatro Game UML Diagram
...
### Design Overview (40+ classes/interfaces):

**Core Entities (4):**

- `Card`, `Deck`, `CardValue`, `Suit` - Basic playing cards

**Hand Evaluation (5):**

- `HandEvaluator` - Poker hand recognition (SRP)
- `HandResult`, `HandType`, `HandUpgradeManager`, `HandUpgrade` - Hand system

**Special Cards (11):**

- `Joker` (abstract) → `ChipJoker`, `MultJoker`, `MultiplierJoker` (OCP)
- `Tarot` (abstract) → `InstantTarot`, `TargetedTarot` (OCP)
- `Planet` - Hand upgrades

**Scoring System (4):**

- `ScoreCalculator` - Enforces strict order (SRP)
- `ScoreContext`, `ScoreResult`, `ScoreBreakdown`

**Blind System (6):**

- `Blind` (abstract) → `SmallBlind`, `BigBlind`, `BossBlind` (LSP)
- `BlindGenerator`, `BlindModifier`, `BossType`

**Game Management (2):**

- `GameState` - Central state (SRP)
- `GameController` - Orchestrator (SRP)

**Shop System (4):**

- `Shop`, `ShopItem`, `ShopItemType`, `ShopItemGenerator` (SRP)

**Persistence & Config (3):**

- `GamePersistence`, `GameConfig`, `BalancingConfig`

### SOLID Principles:

✅ **SRP**: Each system isolated (scoring, shop, blinds, persistence)
✅ **OCP**: Abstract Joker/Tarot/Blind for extensions
✅ **LSP**: All blind types substitutable
✅ **ISP**: Focused interfaces per component
✅ **DIP**: Controller depends on abstractions

Ready for any adjustments or the next application! 🃏