...
## 3.22. Components - Notification {toggle="true"}
		### **Prompt** {toggle="true"}
			```markdown
# GLOBAL CONTEXT
...
- [Improvement 2]
		...
		### Respuesta {toggle="true"}
			```markdown
# Cartographic Project Manager - Notification Components Implementation
...
**Purpose:** Display and manage user notifications with filtering, grouping, and infinite scroll
...
```
src/presentation/components/notification/
├── index.ts                # Barrel export
├── NotificationItem.vue    # Single notification row
└── NotificationList.vue    # Full notification panel
```
...
**Total:** 2 components + barrel export, ~1,110 lines, 0 compilation errors

**Adjustments from specification:**
- Only 6 `NotificationType` values exist in the actual enum (not 10)
- `relatedEntityType` not in DTO; entity type inferred from notification type
- `FileUploadProgressDto` defined inline in FileUploader (didn't exist in app DTOs)
...
```typescript
// Actual enum vs specification
const TYPE_CONFIG: Record<NotificationType, { icon: string; color: string; label: string }> = {
  [NotificationType.NEW_MESSAGE]:       { icon: '💬', color: 'badge-primary', label: 'Mensaje' },
  [NotificationType.NEW_TASK]:          { icon: '✅', color: 'badge-primary', label: 'Tarea' },
  [NotificationType.TASK_STATUS_CHANGE]:{ icon: '🔄', color: 'badge-info',    label: 'Estado' },
  [NotificationType.FILE_RECEIVED]:     { icon: '📎', color: 'badge-info',    label: 'Archivo' },
  [NotificationType.PROJECT_ASSIGNED]:  { icon: '📝', color: 'badge-info',    label: 'Proyecto' },
  [NotificationType.PROJECT_FINALIZED]: { icon: '🎉', color: 'badge-success', label: 'Finalizado' }
};

// Removed (not in enum): TASK_CONFIRMED, TASK_REJECTED, DEADLINE_APPROACHING, DEADLINE_OVERDUE
```
...
### 1. NotificationItem (~413 lines)

**Purpose:** Single notification row with icon, message, timestamp, and hover actions
...
```
┌─ [UNREAD dot] ──────────────────────────────────────────┐
│  [icon]  Titulo de la notificación          [14:30]     │
│          Descripción del mensaje aquí...                 │
│          [Tipo badge]              [✓ Marcar] [🗑 Eliminar] │
└──────────────────────────────────────────────────────────┘
```

**Unread state:**
```css
/* Unread: left accent border + light background */
.notification-item--unread {
...
```typescript
const truncatedMessage = computed(() => {
  const limit = props.compact ? 60 : 120;
  return notification.message.length > limit
    ? notification.message.slice(0, limit) + '...'
...
**Entity type inference (no relatedEntityType in DTO):**
```typescript
const entityLabel = computed(() => {
  switch (notification.type) {
    case NotificationType.NEW_MESSAGE:        return 'Proyecto';
    case NotificationType.NEW_TASK:           return 'Tarea';
    case NotificationType.TASK_STATUS_CHANGE: return 'Tarea';
    case NotificationType.FILE_RECEIVED:      return 'Archivo';
    case NotificationType.PROJECT_ASSIGNED:   return 'Proyecto';
    case NotificationType.PROJECT_FINALIZED:  return 'Proyecto';
    default: return null;
  }
});
```
...
**Hover actions:**
```typescript
// Show mark-read and delete buttons on hover
const isHovered = ref(false);

// Actions only visible when hovered (opacity transition)
// Mark as read: only shown if !notification.isRead
// Delete: always shown
```
...
### 2. NotificationList (~697 lines)

**Purpose:** Full notification panel with filters, date grouping, infinite scroll, and empty states

**Props:**
```typescript
interface Props {
  compact?: boolean;    // Dropdown mode (360×400px max)
}
```

**Emits:** `close`, `navigate`

**Layout (compact mode = dropdown):**
```
┌─ HEADER ─────────────────────────────────────────────────┐
│  🔔 Notificaciones          [3 sin leer]       [Cerrar ×] │
└──────────────────────────────────────────────────────────┘
┌─ FILTERS ────────────────────────────────────────────────┐
│ [Todos] [Sin leer] [Tareas] [Mensajes] [Proyectos]        │
│                                          [✓ Marcar todas] │
└──────────────────────────────────────────────────────────┘
┌─ GROUPS ─────────────────────────────────────────────────┐
│ ─── Hoy ──────────────────────────────────────────────── │
...
**Filter options:**
```typescript
type FilterOption = 'all' | 'unread' | 'tasks' | 'messages' | 'projects';
...
// Filter to types mapping
const FILTER_TYPES: Record<FilterOption, NotificationType[]> = {
  all:      [],
  unread:   [],
  tasks:    [NotificationType.NEW_TASK, NotificationType.TASK_STATUS_CHANGE],
  messages: [NotificationType.NEW_MESSAGE],
  projects: [NotificationType.PROJECT_ASSIGNED, NotificationType.PROJECT_FINALIZED, NotificationType.FILE_RECEIVED]
};
```
...
**Loading skeletons:**
```vue
<!-- 4 skeleton items during initial load -->
<div v-if="isLoading" class="skeleton-list">
  <div v-for="i in 4" :key="i" class="skeleton-item animate-pulse">
    <div class="skeleton-icon" />
    <div class="skeleton-content">
...
<AppEmptyState
  ...
  :description="`No hay notificaciones de tipo '${activeFilter}'`"
>
  <template #action>
    <AppButton size="sm" variant="ghost" @click="activeFilter = 'all'">
      Ver todas
    </AppButton>
  </template>
</AppEmptyState>
...
<AppEmptyState
  ...
  description="Aquí aparecerán tus notificaciones cuando haya actividad"
/>
```
...
### Header Badge (AppHeader.vue)

```vue
<script setup lang="ts">
import { useNotifications } from '@/presentation/composables';
import { NotificationList } from '@/presentation/components/notification';
...
<template>
  <button class="notification-trigger" @click="showPanel = !showPanel">
    🔔
  ...
  <Transition name="slide-down">
    <NotificationList
      ...
      @navigate="showPanel = false"
    />
  </Transition>
</template>
```

### Full Page (NotificationsView.vue)
...
## Also Fixed: FileUploader.vue

The `FileUploadProgressDto` was referenced but didn't exist in the application DTOs. Fixed by defining it inline:

```typescript
// Defined inline in FileUploader.vue
...
|-----------|-------|-------|-------|-------------|
| NotificationItem | ~413 | 2 | 3 | Icons, unread state, hover actions, timestamps |
| NotificationList | ~697 | 1 | 2 | Filters, grouping, infinite scroll, skeletons |
...
1. **Push Notifications** — Browser `Notification` API integration on new items.

2. **Notification Preferences** — Per-type toggle settings panel.

3. **Snooze** — Temporarily hide notification; resurface after N hours.

4. **Bulk Delete** — Select multiple notifications for batch removal.

5. **Rich Previews** — Show task/message snippets inline.

6. **Sound Alerts** — Optional audio on new notifications.

7. **Animation** — Slide-in animation for new real-time notifications.

8. **Notification Badges on Nav** — Per-section unread counts in sidebar.

9. **Email Digest** — Summary of unread notifications by email (backend feature).

10. **WhatsApp Preview** — Indicator showing which notifications were sent via WhatsApp.
...
| Module | Files | Lines |
|--------|-------|-------|
| Infrastructure Repositories | 9 | ~2,670 |
| Shared Layer | 3 | ~1,755 |
| Presentation Styles | 2 | ~1,500 |
| Presentation Stores | 6 | ~2,250 |
| Presentation Composables | 6 | ~1,450 |
| Presentation Router | 1 | ~620 |
| Common Components | 11 | ~865 |
| Project Components | 3 | ~1,550 |
| Task Components | 4 | ~1,780 |
| Message Components | 4 | ~1,620 |
| File Components | 3 | ~1,225 |
| Notification Components | 3 | ~1,115 |
| **Grand Total** | **~55 files** | **~18,400 lines** |

---

## Conclusion

Both notification components are **production-ready**:

✅ **6 notification types** correctly mapped to actual enum  
✅ **Unread visual state** — Left border, background, dot indicator  
✅ **Date grouping** — Hoy / Ayer / Esta semana / Month Year  
✅ **5 filter categories** — All, Unread, Tasks, Messages, Projects  
✅ **IntersectionObserver** — Infinite scroll at bottom sentinel  
✅ **Sticky group headers** — Float above scrolling content  
✅ **Custom scrollbar** — Thin, styled for compact mode  
✅ **Loading skeletons** — 4 items matching real content  
✅ **Compact + full modes** — Dropdown (360px) and full-page  
