...
## 3.19. Components - Task  {toggle="true"}
		### **Prompt** {toggle="true"}
			```markdown
# GLOBAL CONTEXT
...
- [Improvement 2]
		...
		### Respuesta {toggle="true"}
			```markdown
# Cartographic Project Manager - Task Components Implementation
...
**Purpose:** Task display, creation/editing, filtered lists, and audit trail visualization
...
├── TaskCard.vue       # Individual task card with quick actions
├── TaskForm.vue       # Create/edit form with workflow
├── TaskList.vue       # Filterable/sortable task list
└── TaskHistory.vue    # Timeline audit trail
```
...
**Total:** 4 components, ~1,780 lines, 0 compilation errors

---

## Component Details

### 1. TaskCard (~350 lines)

**Purpose:** Compact task card with priority bar, status badge, quick transitions, and action menu
...
interface Props {
  ...
  showActions?: boolean;
  selected?: boolean;
  draggable?: boolean;    // Kanban drag-and-drop support
}
```

**Emits:** `click`, `edit`, `delete`, `status-change`
...
│                                              │
│ 👤 Assignee   📅 31/12   📎 2    [⋮ menu] │
│                                              │
│ [→ In Progress] [→ Performed]  (quick btns) │
└──────────────────────────────────────────────┘
```
...
const PRIORITY_BAR_COLORS = {
  ...
  [TaskPriority.MEDIUM]: '#ca8a04',  // Yellow
  [TaskPriority.LOW]:    '#16a34a'   // Green
};
```
...
```typescript
const validTransitions = computed(() =>
  (TASK_CONFIG.STATUS_TRANSITIONS[task.status] as unknown as TaskStatus[])
    .slice(0, 2)  // Show max 2 quick buttons
);
```
...
// "Vencido hace 3 días" / "Vence hoy" / "31/12/2025"
```

**Completed state:** 70% opacity on entire card

**Permission checks via `useAuth`:**
```typescript
const canEdit = computed(() =>
...
const canDelete = computed(() =>
  task.creatorId === authStore.userId || authStore.isAdmin
);
```

---

### 2. TaskForm (~630 lines)

**Purpose:** Create and edit form with 5-state workflow management and confirmation flow
...
```typescript
interface Emits {
  submit: [data: CreateTaskDto | UpdateTaskDto | ChangeTaskStatusDto | ConfirmTaskDto];
  cancel: [];
}
```
...
| Field | Type | Validation | Mode |
|-------|------|-----------|------|
| Description | textarea | Required, max 1000 chars | Both |
| Assignee | select | Required | Both |
| Priority | select | Required | Both |
| Due date | date | Optional, future recommended | Both |
| Comment | textarea | Optional, max 2000 chars | Edit only |
| Status | workflow buttons | Valid transitions only | Edit only |

**Status change section (edit mode):**
```
Current: [IN PROGRESS]

Change to:
[→ Pending] [→ Partial] [→ Performed]
```

**Confirmation flow (PERFORMED → COMPLETED):**
```typescript
// Only assignee sees this section when task is PERFORMED
if (task.status === TaskStatus.PERFORMED && task.assigneeId === authStore.userId) {
...
**Character counters:**
```typescript
// Description counter (shows at 80%+)
descriptionCounter: `${description.length}/1000`

// Comment counter (shows at 80%+)
commentCounter: `${comment.length}/2000`
```

**Read-only state:** Completed tasks show a notice and disable all fields

**Validation (blur-triggered):**
```typescript
const validateDescription = () => {
  touched.description = true;
  if (!description.value.trim()) errors.description = 'La descripción es obligatoria';
  else if (description.value.length > 1000) errors.description = 'Máximo 1000 caracteres';
...
```typescript
// Create
emit('submit', { projectId, creatorId, assigneeId, description, priority, dueDate });

// Update
emit('submit', { assigneeId, description, priority, dueDate });

// Status change
emit('submit', { taskId, newStatus, userId, comment });

// Confirm/reject
emit('submit', { taskId, userId, comment });
```

---

### 3. TaskList (~450 lines)

**Purpose:** Full-featured task list with filtering, sorting, view modes, and loading/empty states
...
**Emits:** `click`, `edit`, `delete`, `status-change`, `create`, `filter-change`

**Filter Controls:**
```
[Status ▼]  [Priority ▼]  [Assignee ▼]  [Sort ▼]  [List/Group] [Clear filters]
                                                                [+ New Task]
```

**Status filter options:** All, Pending, In Progress, Partial, Performed, Completed

**Priority filter options:** All, Urgent, High, Medium, Low

**Sort options:**
```typescript
const SORT_OPTIONS = [
  { value: 'dueDate-asc',    label: 'Fecha de entrega ↑' },
  { value: 'dueDate-desc',   label: 'Fecha de entrega ↓' },
  { value: 'priority-asc',   label: 'Prioridad ↑' },
...
**View modes:**
- **List:** Single column, all tasks sorted
- **Grouped:** Separate section per status, count badge per group

**Loading state:** 4 animated skeleton cards matching TaskCard structure

**Empty states:**
```vue
<!-- No tasks at all -->
<AppEmptyState icon="📋" title="No hay tareas" description="Crea la primera tarea...">
  <template #action><AppButton @click="emit('create')">Nueva Tarea</AppButton></template>
</AppEmptyState>

<!-- Filtered with no results -->
<AppEmptyState compact icon="🔍" title="Sin resultados" description="Prueba con otros filtros">
  <template #action><AppButton @click="clearFilters">Limpiar filtros</AppButton></template>
</AppEmptyState>

<!-- Per-group empty state (grouped mode) -->
<p class="group-empty">No hay tareas pendientes</p>
```
...
const filteredTasks = computed(() => {
  ...
  if (assigneeFilter.value)
    result = result.filter(t => t.assigneeId === assigneeFilter.value);

  return sortTasks(result);
});
```

**Grouped view computation:**
```typescript
...
### 4. TaskHistory (~350 lines)

**Purpose:** Timeline visualization of task changes (audit trail)
...
**Visual Structure:**
```
  🟢 ●─── John Admin created this task          2h ago
        📅 01/01/2025
     │
  🔵 ●─── John Admin changed the status         1h ago
        PENDING → IN_PROGRESS
     │
  🔴 ●─── Maria Client rejected the task        30m ago
        💬 "Missing data on sector B"
     │
  🟢 ●─── John Admin confirmed and completed    5m ago
```
...
const DOT_COLORS: Record<string, string> = {
  ...
  status_changed:    '#2563eb',  // Blue
  rejected:          '#dc2626',  // Red
  reassigned:        '#7c3aed',  // Purple
  updated:           '#6b7280',  // Gray
  file_attached:     '#0891b2',  // Cyan
  file_removed:      '#dc2626'   // Red
};
```

**Action descriptions (Spanish):**
```typescript
function describeAction(entry: TaskHistoryEntryDto): string {
  const actions: Record<string, string> = {
    'STATUS_CHANGE':     'cambió el estado',
    'CREATED':           'creó la tarea',
    'CONFIRMED':         'confirmó y completó la tarea',
    'REJECTED':          'rechazó y devolvió la tarea',
    'ASSIGNMENT_CHANGE': 'reasignó la tarea',
    'PRIORITY_CHANGE':   'cambió la prioridad',
    'DUE_DATE_CHANGE':   'cambió la fecha de entrega',
    'FILE_ATTACHED':     'adjuntó un archivo',
    'FILE_REMOVED':      'eliminó un archivo'
  };
  return actions[entry.action] ?? 'realizó un cambio';
}
```

**Value change display:**
```typescript
// "PENDING → IN_PROGRESS" shown as:
<span class="old-value">Pendiente</span>
<span class="arrow">→</span>
<span class="new-value">En progreso</span>
```

**Value formatting:**
```typescript
function formatValue(value: string, action: string): string {
  // Status enum → human label
  if (Object.values(TaskStatus).includes(value as TaskStatus))
    return STATUS_LABELS[value as TaskStatus];

  // Priority enum → human label
...
**Relative timestamps:**
```typescript
// "Hace 5 min" / "Hace 2h" / "Hace 3 días" / "01/01/2025"
```

**Loading state:** 3 animated skeleton entries with connecting line

**Empty state:**
```vue
<AppEmptyState compact icon="🕒" title="Sin historial" description="Aún no se han registrado cambios" />
```

---

## Status/Priority Display Values

Used consistently across all 4 components:

```typescript
const STATUS_LABELS: Record<TaskStatus, string> = {
  [TaskStatus.PENDING]:     'Pendiente',
  [TaskStatus.IN_PROGRESS]: 'En progreso',
  [TaskStatus.PARTIAL]:     'Parcial',
  [TaskStatus.PERFORMED]:   'Realizada',
...
<template>
  ...
  <div v-if="activeTab === 'tasks'">
    <TaskList
      :tasks="taskStore.getTasksByProject(projectId)"
  ...
  <!-- Task detail modal -->
  <AppModal v-model="showTaskModal" :title="selectedTask?.description" size="lg">
    <TaskForm
      :task="selectedTask"
      :project-id="projectId"
...
| Component | Lines | Props | Emits | Computed |
|-----------|-------|-------|-------|----------|
| TaskCard | ~350 | 5 | 4 | 7 |
| TaskForm | ~630 | 4 | 2 | 6 |
| TaskList | ~450 | 5 | 6 | 5 |
| TaskHistory | ~350 | 2 | 0 | 4 |
| **Total** | **~1,780** | **16** | **12** | **22** |

---

## Possible Future Improvements

1. **Icon Library** — Replace emoji with Lucide Vue icons.

2. **Virtual Scrolling** — `vue-virtual-scroller` for 100+ task lists.

3. **Full Kanban Board** — Drop zones with `@vueuse/gesture` or `vue-draggable-plus`.

4. **Bulk Operations** — Checkbox multi-select with batch status change/delete.

5. **Advanced Filters** — Due date range picker, description text search.

6. **Task Templates** — Save and reuse common task configurations.

7. **Attachment Previews** — Thumbnail images in TaskForm file list.

8. **Keyboard Shortcuts** — Arrow key navigation, `c` to complete, `e` to edit.

9. **Undo/Redo** — For status changes and deletions via toast with "Deshacer" action.

10. **Task Dependencies** — "Bloqueado por" / "Bloquea a" relationship display.

---

## Conclusion

All 4 task components are **production-ready**:

✅ **5-state workflow** enforced via TASK_CONFIG.STATUS_TRANSITIONS  
✅ **Confirmation flow** — Assignee-only PERFORMED → COMPLETED  
✅ **Permission-based UI** — Edit/delete/confirm gated by role  
✅ **Dual view modes** — List and grouped-by-status  
✅ **Blur-triggered validation** — Non-aggressive error display  
✅ **Timeline audit trail** — Color-coded action history  
✅ **Loading skeletons** — Match real content structure  
✅ **Spanish locale** — All labels, dates, and messages  
