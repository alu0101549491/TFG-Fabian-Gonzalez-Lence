...
```typescript
// tests/unit/models/blinds/blinds.test.ts
/**
...
describe('Blinds System Unit Tests', () => {
  ...
  describe('BossBlind Class', () => {
    ...
    describe('getScoreGoal() - Boss-Specific Modifiers', () => {
      ...
      it('should return 1600 for The Needle round 2 (800 × 2.0 × 0.5)', () => {
...
- All 5 BossType enum values verified with display names and descriptions
- BlindModifier constructor and factory method fully tested with all boss variants
- Small/Big/BossBlind constructors validated with edge cases (level/round boundaries)
- All 8 round base values verified against Balatro specification (\[300, 800, ..., 50000\])
- BlindGenerator pattern verification for all 24 levels with deterministic boss selection
- Integration tests covering complete progression and modifier application
### 2. Critical Implementation-Accurate Testing
- **Fixed Base Goal Formula**: Tests use actual lookup table values (not exponential formula from outdated spec)
- **Boss Modifier Timing**: Verifies goalMultiplier applies AFTER base ×2.0 multiplier (e.g., The Wall = 300 × 2 × 4 = 2400)
- **Round Capping**: Confirms rounds beyond 8 use round 8 values (50000 base)
- **The Mouth Behavior**: Tests initial `null` allowedHandTypes and post-play locking via `setAllowedHandType()`
- **Blind Type Identifiers**: Tests `getBlindType()` returning string identifiers ("SmallBlind", etc.) not display names
### 3. Advanced Test Patterns
- **Deterministic Boss Selection**: Controls `Math.random` for predictable boss generation in tests
- **Pattern Verification Loops**: Validates Small→Big→Boss pattern across all 24 levels programmatically
- **Round Calculation Verification**: Cross-checks level→round mapping via score goal values
- **Boss History Testing**: Verifies avoidance of last 2 used bosses in selection algorithm
- **Floating Point Validation**: Ensures all goals are integers without precision errors
### 4. Edge Case Coverage
- Level 0/negative validation errors
- Round 0/negative validation errors
- Rounds beyond 8 (capped at round 8 values)
- Large levels (100+) with correct round calculation
- Boss modifier combinations (The Needle: 1 hand + 0.5× goal)
- The Mouth hand type locking workflow
- Multiplier/divisor edge cases (flooring to 1 for mult)
### 5. Integration Testing
- Complete 24-level progression verification
- Blind type ordering (Small \< Big \< Boss within round)
- Round progression monotonicity (goals increase each round)
- Boss modifier application correctness across all 5 types
- Score goal integer validation for all blinds
### Execution Instructions
```bash
# Run all blind tests
npm test -- tests/unit/models/blinds/blinds.test.ts

# Run with coverage report
npm test -- --coverage tests/unit/models/blinds/blinds.test.ts

# Run specific boss type tests
npm test -- -t "The Wall" tests/unit/models/blinds/blinds.test.ts
npm test -- -t "BlindGenerator" tests/unit/models/blinds/blinds.test.ts

# Watch mode for TDD
npm test -- --watch tests/unit/models/blinds/blinds.test.ts

```
### Critical Implementation Notes
1. **Base Goal Formula**: Uses fixed lookup table `[300, 800, 2000, 5000, 11000, 20000, 35000, 50000]` not exponential formula
2. **Boss Goal Calculation**: `baseGoal × 2.0 × modifier.goalMultiplier` (modifier applied after base ×2)
3. **The Mouth Initial State**: `allowedHandTypes = null` until first hand played (then locked via `setAllowedHandType`)
4. **Round Capping**: Rounds \>8 use round 8 values (50000 base)
5. **Blind Type Identifiers**: `getBlindType()` returns string identifiers ("SmallBlind") not display names
This test suite provides production-grade validation for Mini Balatro's blinds system with 100% alignment to the provided implementation, covering all specification requirements while accurately reflecting actual code behavior. All 5 boss types, 3 blind classes, and BlindGenerator pattern logic are verified against Balatro's official progression values.
