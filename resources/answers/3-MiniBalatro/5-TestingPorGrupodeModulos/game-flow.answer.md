...
/** Forces level completion by setting accumulated score to goal */
...
function setupShopWithItems(controller: GameController, items: ShopItem[]): void {
  if (!controller.isInShopMode()) {
...
describe('Game Flow Integration Tests', () => {
  ...
  describe('Basic Game Flow', () => {
    ...
    it('should progress through first 3 levels with correct blind types and goals', async () => {
      ...
      expect(bossGoal).toBeGreaterThanOrEqual(600); // Minimum boss goal
    });
  ...
  describe('Victory Path', () => {
    it('should trigger victory after completing level 24', async () => {
      ...
      await controller.exitShop(); // Advance to level 24

      // Complete level 24
      forceCompleteLevel(controller);
      await controller.exitShop(); // This should trigger victory
    ...
    it('should accumulate sufficient money through 8 rounds to purchase necessary items', async () => {
      ...
      expect(finalMoney).toBeGreaterThan(50); // Starting with $5, should have substantial accumulation
    });
  });
  ...
  describe('Joker Integration', () => {
    it('should persist joker across levels and affect scoring', async () => {
      ...
      const jokerItem = createJokerShopItem('test-joker', 'Test Joker', '+4 mult', 4);
      setupShopWithItems(controller, [jokerItem]);
    ...
    it('should stack multiple jokers correctly in score calculation', async () => {
      ...
      const items = [
        createJokerShopItem('j1', 'Joker 1', '+4 mult', 4),
        createJokerShopItem('j2', 'Joker 2', '+4 mult', 4),
        createJokerShopItem('j3', 'Joker 3', '+4 mult', 4)
      ];
      setupShopWithItems(controller, items);

      // Purchase all three jokers
    ...
    it('should award Golden Joker economic bonus on level completion', async () => {
      ...
      setupShopWithItems(controller, [goldenItem]);
    ...
    it('should apply Hiker permanent upgrade to played cards', async () => {
      ...
      setupShopWithItems(controller, [hikerItem]);
      ...
      const result = await controller.playSelectedHand();
      expect(result.totalScore).toBeGreaterThan(15); // Should be higher than base single card
    });
  });
  ...
  describe('Planet Integration', () => {
    it('should apply Pluto upgrade to High Card hands', async () => {
      ...
      setupShopWithItems(controller, [plutoItem]);
      ...
      expect(result.baseChips).toBe(15); // Upgraded base chips
      expect(result.baseMult).toBe(2);    // Upgraded base mult
    ...
    it('should accumulate multiple planet upgrades to same hand type', async () => {
      ...
      setupShopWithItems(controller, [pluto1, pluto2]);
    ...
    it('should upgrade different hand types with appropriate planets', async () => {
      ...
      setupShopWithItems(controller, [pluto, mercury]);
      ...
      expect(result.baseChips).toBe(15); // Pluto applied
  ...
  describe('Tarot Integration', () => {
    it('should modify card permanently with The Empress targeted tarot', async () => {
      ...
      setupShopWithItems(controller, [empressItem]);
    ...
    it('should double money with The Hermit instant tarot', async () => {
      ...
      setupShopWithItems(controller, [hermitItem]);
    ...
    it('should duplicate card with Death tarot and add to deck', async () => {
      ...
      setupShopWithItems(controller, [deathItem]);
      ...
      expect(gameState.getDeckRemaining()).toBe(originalDeckSize + 1);
  ...
  describe('Save/Load Integration', () => {
    it('should save and restore complete game state including inventory', async () => {
      ...
      const jokerItem = createJokerShopItem('test-joker', 'Test Joker', '+4 mult', 4);
      const plutoItem = createPlanetShopItem('Pluto', HandType.HIGH_CARD, 10, 1);
      setupShopWithItems(controller, [jokerItem, plutoItem]);
  ...
  describe('Shop Mechanics Integration', () => {
    ...
    it('should handle joker inventory full by requiring replacement', async () => {
      ...
      const jokerItem = createJokerShopItem('new-joker', 'New Joker', '+4 mult', 4);
      setupShopWithItems(controller, [jokerItem]);
  ...
  describe('Multi-System Integration', () => {
    it('should combine joker + planet + tarot bonuses for massive scores', async () => {
      ...
      const items = [
        createJokerShopItem('mult-joker', 'Mult Joker', '+4 mult', 4),
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle maximum inventory complexity (5 jokers, 2 tarots)', async () => {
      ...
      // 5 jokers
      for (let i = 0; i < 5; i++) {
        items.push(createJokerShopItem(`j${i}`, `Joker ${i}`, '+4 mult', 4));
      }
      ...
      setupShopWithItems(controller, items.slice(0, 4)); // Shop holds 4 at a time
      ...
      setupShopWithItems(controller, items.slice(4, 8));
    ...
    it('should recover gracefully from save/load during active hand', async () => {
      ...
      const loadedState = newController.getGameState()!;
      expect(loadedState.getSelectedCards()).toHaveLength(2);
...
```
## Key Features of This Integration Test Suite
### 1. Realistic Game Flow Simulation
- Tests complete level cycles (play → shop → next level)
- Simulates 24-level victory path with strategic purchases
- Validates defeat conditions with insufficient score
- Tests rapid progression without state corruption
### 2. Multi-System Integration Validation
- **Joker + Planet + Tarot Synergy**: Tests combined bonuses stacking correctly
- **Boss Blind Mechanics**: Validates all 5 boss modifiers (The Wall, Water, Mouth, Needle, Flint)
- **Economic Systems**: Golden Joker bonuses, shop rerolls, purchase limits
- **Persistence**: Save/load with inventory, shop state, and active hands
### 3. Implementation-Accurate Testing
- Uses `debugForceBoss()` to test specific boss mechanics deterministically
- Sets shop items directly via `setupShopWithItems()` for reliable purchase tests
- Uses `forceCompleteLevel()` helper to bypass hand-playing for flow tests
- Respects actual money limits and shop mechanics (\$3 reroll, \$5 jokers)
### 4. Comprehensive Scenario Coverage
- **Beginner Path**: Conservative play with basic purchases
- **Advanced Strategy**: Joker synergies, planet optimization
- **Boss Challenges**: Special mechanics for each boss type
- **Economic Strategy**: Golden Joker income, frequent rerolls
- **Failure Cases**: Bad RNG, wrong purchases, money management
### 5. Edge Case Handling
- Rapid level completions (state stability)
- Maximum inventory complexity (5 jokers + 2 tarots)
- Save/load during active hand selection
- Inventory full requiring replacement
- Discard attempts with The Water boss (0 discards)
### 6. Callback Verification
- All 8 GameController callbacks tested (`onVictory`, `onDefeat`, etc.)
- Callbacks triggered at correct game state transitions
- Defeat information includes boss details when applicable
...
npm test -- -t "Boss Blind" tests/integration/game-flow.test.ts
npm test -- -t "Multi-System" tests/integration/game-flow.test.ts
...
1. **Deterministic Boss Testing**: Uses `debugForceBoss()` to test specific boss mechanics reliably
2. **Shop Setup**: Replaces random shop items with known items for purchase tests
3. **Level Skipping**: Uses `forceCompleteLevel()` to bypass hand-playing for flow validation
4. **State Preservation**: Verifies all critical state survives save/load cycles
5. **Callback Safety**: All callbacks wrapped in try-catch to prevent test crashes
6. **Money Management**: Respects actual game economy (\$5 start, \$3 reroll cost)
7. **Inventory Limits**: Tests 5-joker and 2-tarot limits with replacement logic
This integration test suite validates that all Mini Balatro systems work together seamlessly to deliver the complete game experience, from first hand to final victory.
