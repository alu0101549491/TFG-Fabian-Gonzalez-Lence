...
```markdown
// tests/views/hangman-renderer.test.ts
...
describe('HangmanRenderer', () => {
  ...
  describe('Integration', () => {
    ...
    it('should support restart flow: render(6) → clear() → render(0)', () => {
      // ARRANGE: Complete hangman drawn
      hangmanRenderer.render(6);
      ...
      // ASSERT: Should clear and draw initial state
      expect(mockContext.clearRect).toHaveBeenCalledTimes(2); // Once for clear(), once for render(0)
    });
...
> jest tests/views/hangman-renderer.test.ts

 FAIL  tests/views/hangman-renderer.test.ts
  HangmanRenderer
    constructor
      ✓ should initialize with valid canvas ID (4 ms)
      ✓ should throw error when canvas element not found (8 ms)
      ✓ should throw error when element is not a canvas (2 ms)
      ✓ should throw error when 2D context unavailable (2 ms)
      ✓ should store reference to canvas element (1 ms)
      ✓ should store reference to 2D context (1 ms)
      ✓ should validate canvas element type (1 ms)
    render
      ✓ should clear canvas before drawing (1 ms)
      ✓ should draw only gallows for state 0 (1 ms)
      ✓ should draw gallows and head for state 1 (1 ms)
      ✓ should draw progressively more parts as attempts increase (1 ms)
      ✓ should draw complete hangman for state 6 (1 ms)
      ✓ should draw gallows + head + body for state 2
      ✓ should draw gallows + head + body + left arm for state 3 (1 ms)
      ✓ should draw gallows + head + body + both arms for state 4
      ✓ should draw gallows + head + body + both arms + left leg for state 5 (1 ms)
      ✓ should clamp attempts to valid range (0-6) (1 ms)
      ✓ should draw same state consistently across multiple calls
      ✓ should draw gallows for all states (0-6) (1 ms)
      ✓ should draw head for states 1-6 (not for state 0)
      ✓ should draw body for states 2-6 (not for states 0-1) (1 ms)
      ✓ should draw left arm for states 3-6 (not for states 0-2) (1 ms)
      ✓ should draw right arm for states 4-6 (not for states 0-3) (1 ms)
      ✓ should draw left leg for states 5-6 (not for states 0-4)
      ✓ should draw right leg only for state 6 (not for states 0-5) (1 ms)
    clear
      ✓ should call clearRect with canvas dimensions (1 ms)
      ✓ should allow rendering after clear (2 ms)
      ✓ should clear canvas completely (1 ms)
      ✓ should be safe to call clear multiple times
      ✓ should work after constructor (1 ms)
    Edge Cases
      ✓ should handle render with negative attempts
      ✓ should handle render with attempts > 6
      ✓ should render(0) multiple times idempotently (1 ms)
      ✓ should render(6) multiple times idempotently
      ✓ should handle non-sequential render sequence: 0 → 3 → 6 (1 ms)
      ✓ should handle reverse render sequence: 6 → 0
      ✓ should use correct canvas dimensions (1 ms)
      ✓ should maintain cumulative drawing across different states
    Canvas API Integration
      ✓ should set correct line styling before drawing (1 ms)
      ✓ should call beginPath before each drawing operation
      ✓ should call stroke after each drawing operation
      ✓ should call moveTo and lineTo for line drawings
      ✓ should call arc for head drawing
      ✓ should follow correct Canvas API sequence (1 ms)
      ✓ should clear canvas using correct dimensions
    Integration
      ✓ should work with standard canvas ID "hangman-canvas" (1 ms)
      ✓ should support typical game flow: render(0) → render(1) → ... → render(6)
      ✕ should support restart flow: render(6) → clear() → render(0) (1 ms)
      ✓ should work with multiple HangmanRenderer instances (different canvases) (1 ms)
      ✕ should handle container with existing content (1 ms)
      ✓ should work with GameModel attempt values (0-6) (1 ms)

  ● HangmanRenderer › Integration › should support restart flow: render(6) → clear() → render(0)
    ...
    Received number of calls: 3
      ...
      646 |       
      647 |       // ASSERT: Should clear and draw initial state
    > 648 |       expect(mockContext.clearRect).toHaveBeenCalledTimes(2); // Once for clear(), once for render(0)
          |                                     ^
      649 |     });
      650 |
      651 |     it('should work with multiple HangmanRenderer instances (different canvases)', () => {

      at Object.<anonymous> (tests/views/hangman-renderer.test.ts:648:37)

  ● HangmanRenderer › Integration › should handle container with existing content

    expect(jest.fn()).toHaveBeenCalledWith(...expected)

    Expected: 0, 0, 400, 400

    Number of calls: 0

      727 |       
      728 |       // ASSERT
    > 729 |       expect(mockContextInContainer.clearRect).toHaveBeenCalledWith(0, 0, 400, 400);
          |                                                ^
      730 |     });
      731 |
      732 |     it('should work with GameModel attempt values (0-6)', () => {

      at Object.<anonymous> (tests/views/hangman-renderer.test.ts:729:48)

Test Suites: 1 failed, 1 total
Tests:       2 failed, 49 passed, 51 total
Snapshots:   0 total
Time:        0.429 s, estimated 1 s
Ran all test suites matching /tests\/views\/hangman-renderer.test.ts/i.
```
**Respuesta:**
The test failures are due to two issues in the test implementation:
1. **Restart Flow Test:** The test expects `clearRect` to be called twice (once by `clear()`, once by `render(0)`), but it's being called three times. This happens because the initial `render(6)` at the beginning also calls `clearRect`.
2. **Container with Existing Content Test:** The test creates a new canvas (`canvasInContainer`) and a new mock context (`mockContextInContainer`), but the `HangmanRenderer` constructor is still using the original global `mockContext` because the global `canvas.getContext` mock is used. The new mock context is never called.
Here's the corrected test file with fixes for both issues:
```typescript
// tests/views/hangman-renderer.test.ts
import {HangmanRenderer} from '@views/hangman-renderer';
