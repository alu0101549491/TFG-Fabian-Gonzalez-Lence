...
I need to fix the failing tests based on the actual implementation of the `parseTime` function in the original code. Let me analyze the `parseTime` function again:
```typescript
...
1. For `'1a:30'`: `parts[0]` is `'1a'`, `parseInt('1a', 10)` returns `1` (not NaN), so it returns `(1 * 60) + 30 = 90`
2. For `'01:3b'`: `parts[1]` is `'3b'`, `parseInt('3b', 10)` returns `3` (not NaN), so it returns `(1 * 60) + 3 = 63`
3. For `' 01:30'`: `parts[0]` is `' 01'`, `parseInt(' 01', 10)` returns `1` (parseInt handles leading whitespace), so it returns `(1 * 60) + 30 = 90`
The function uses `parseInt`, which is quite permissive and extracts numeric values from the beginning of strings. So my test expectations were wrong.
  ...
  describe('parseTime', () => {
    ...
    describe('Edge Cases', () => {
      ...
      it('should handle whitespace in string (parseInt handles leading whitespace)', () => {
        ...
        expect(TimeFormatter.parseTime(' 01 : 30 ')).toBe(60); // This will split on colon and have " 01 " and " 30 "
