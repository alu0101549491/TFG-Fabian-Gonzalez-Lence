...
export class MatchGeneratorService {
   ...
   * @param phaseType - Type of phases to generate (MAIN, QUALIFYING, CONSOLATION). Defaults to 'MAIN'
  ...
  public generateMatches(
    ...
    totalRounds: number,
    phaseType: 'MAIN' | 'QUALIFYING' | 'CONSOLATION' = 'MAIN',
  ): MatchGenerationResult {
    switch (bracketType) {
      case BracketType.SINGLE_ELIMINATION:
        return this.generateSingleElimination(bracketId, tournamentId, participantIds, totalRounds, phaseType);
      case BracketType.ROUND_ROBIN:
        return this.generateRoundRobin(bracketId, tournamentId, participantIds, totalRounds, phaseType);
      case BracketType.MATCH_PLAY:
        return this.generateMatchPlay(bracketId, tournamentId, participantIds, phaseType);
      default:
   ...
   * @param phaseType - Type of phases (MAIN, QUALIFYING, or CONSOLATION)
  ...
  private generateSingleElimination(
    ...
    // Calculate bracket size from participant array length (already padded to power of 2 by seeding service)
    // This is important for qualifying brackets where totalRounds < log2(bracketSize)
    const bracketSize = participantIds.length;
    const byeCount = bracketSize - playerCount;
    
    // Create phases (rounds) - use bracketSize for correct naming
    const phaseNames = this.getSingleEliminationPhaseNames(totalRounds, bracketSize);
    for (let round = 1; round <= totalRounds; round++) {
      ...
      // Calculate match count based on bracketSize, not totalRounds
      // This is critical for qualifying brackets where totalRounds < log2(bracketSize)
      phase.matchCount = bracketSize / Math.pow(2, round);
      phase.isCompleted = false;
      phase.phaseType = phaseType; // Set phase type based on bracket (MAIN, QUALIFYING, or CONSOLATION)
      phase.isAutoGenerated = true;
      phases.push(phase);
    }
    
    // Generate Round 1 matches - calculate from bracketSize, not totalRounds
    ...
    for (let round = 2; round <= totalRounds; round++) {
      // Calculate from bracketSize, not totalRounds
  ...
  /**
   * Gets human-readable phase names for single elimination rounds based on bracket size.
   *
   * @param totalRounds - Total number of rounds
   * @param bracketSize - Total bracket capacity (power of 2)
   * @returns Array of phase names
   */
  private getSingleEliminationPhaseNames(totalRounds: number, bracketSize: number): string[] {
    const names: string[] = [];
    
    for (let round = 1; round <= totalRounds; round++) {
      // Calculate how many players are competing in this round
      const playersInRound = bracketSize / Math.pow(2, round - 1);
      
      // Name based on actual participant count, not rounds remaining
      if (playersInRound === 2) {
        names.push('Final');
      } else if (playersInRound === 4) {
        names.push('Semifinals');
      } else if (playersInRound === 8) {
        names.push('Quarterfinals');
      } else if (playersInRound === 16) {
        names.push('Round of 16');
      } else if (playersInRound === 32) {
        names.push('Round of 32');
      } else if (playersInRound === 64) {
        names.push('Round of 64');
      } else if (playersInRound === 128) {
        names.push('Round of 128');
      } else {
